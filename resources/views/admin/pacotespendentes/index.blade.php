    ...
    <div class="modal fade bs-example-modal-lg" tabindex="-1" aria-labelledby="ModalNovoPacote" aria-hidden="true" style="display: none;">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <form class="form-horizontal mt-3" method="POST" action="{{ route('pacotes_pendentes.store') }}" id="formNovoPacote">
                        @csrf
...
document.querySelectorAll('.abrirModal').forEach(item => {
        item.addEventListener('click', event => {
            ...
            const url = "{{ route('pacotes_pendentes.show', ':id') }}".replace(':id', pacoteId);
            fetch(url)
                ...
                .then(data => {
                    ...
                    var novaAction = "{{ route('pacotes_pendentes.update', ['pacotependente' => ':id']) }}".replace(':id', data.id);
